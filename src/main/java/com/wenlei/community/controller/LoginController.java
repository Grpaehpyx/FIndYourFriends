...
*/
    ...
    public String login(String username, String password, String code, boolean rememberme,Model model,HttpServletResponse response,
        ...
        //kap为空，即代表redis中不存在kaptchaOwner对应的验证码 ||
        // 前端传入的code为空
        // 且两者不相等相等（不考虑大小写） 都给login模板返回提示信息
        if(StringUtils.isBlank(kaptcha)||StringUtils.isBlank(code)||!kaptcha.equalsIgnoreCase(code)){
            model.addAttribute("codeMsg","验证码不正确");
            return "/site/login";
        }
        ...
        // 检查账号,密码
        //过期时间，时间到了http请求头不携带这个cookie，并且redis中也要删除
        int expiredSeconds = rememberme ? REMEMBER_EXPIRED_SECONDS : DEFAULT_EXPIRED_SECONDS;
        Map<String, Object> map = userService.login(username, password, expiredSeconds);
        if (map.containsKey("ticket")) {
            Cookie cookie = new Cookie("ticket", map.get("ticket").toString());
            cookie.setPath(contextPath);//设置域路径
            cookie.setMaxAge(expiredSeconds);//设置过期时间
            response.addCookie(cookie);//放入响应体
            return "redirect:/index";
        } else {
            model.addAttribute("usernameMsg", map.get("usernameMsg"));
            model.addAttribute("passwordMsg", map.get("passwordMsg"));
    ...
    //你能退出说明你肯定登入了，前端也加控制，请求肯定会携带cookie，退出-->将status状态改为1（无效）
